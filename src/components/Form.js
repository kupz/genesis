...
export default function Form({code}) {
  ...
  return (
    <div className="form-container">
      <form id="guestForm" onSubmit={e => onSubmit(e)} className="form" method="POST" encType='multipart/form-data'>
        <div>
          <label htmlFor="uploadImage"><div id="form-img" style={{backgroundSize: 'cover', backgroundPosition: 'center', width: '10em', height: '10em'}} className="form-img">Upload selfie</div></label>
          <input required name="image" onChange={e => imageChange(e)} id="uploadImage" accept="image/png, image/jpeg" type="file" style={{ display: "none" }} />
        </div>
        <div>
          ...
          <select required onChange={genderChange} id="isMale" name="isMale" id="">
            <option value={1}>Male</option>
            <option value={0}>Female</option>
            <option value={2}>I'd rather not specify</option>
          </select>
          <select required onChange={isAdultChange} id="isAdult" name="isAdult" id="">
